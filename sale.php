...
<head>
  ...
  <title>Sale — The Nation's Bookstore</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hero-eyebrow { display: inline-flex; align-items: center; gap: .5rem; background: rgba(230,126,34,0.25); border: 1px solid rgba(230,126,34,0.5); color: #f0a899; font-size: .75rem; font-weight: 600; letter-spacing: .18em; text-transform: uppercase; padding: .35rem .9rem; border-radius: 99px; margin-bottom: 1.25rem; }
    ...
    /* SALE STATS BAR */
    .stats-bar { background: var(--sale); color: #fff; padding: .65rem 2rem; display: flex; align-items: center; justify-content: center; gap: 2.5rem; flex-wrap: wrap; font-size: .85rem; font-weight: 500; }
    .stats-bar span { display: flex; align-items: center; gap: .4rem; }
    ...
    .filter-select { padding: .55rem .85rem; border: 1px solid var(--border); border-radius: 8px; background: var(--bg); font-family: var(--font-body); font-size: .875rem; color: var(--ink); outline: none; cursor: pointer; }
    .results-count { font-size: .8rem; color: var(--ink-muted); margin-left: auto; white-space: nowrap; }

    /* MAIN LAYOUT */
    .main-wrap { max-width: 1280px; margin: 0 auto; padding: 2rem; }
    .section-header { display: flex; align-items: baseline; gap: 1rem; margin-bottom: 1.5rem; }
    .section-title { font-family: var(--font-display); font-size: 1.5rem; font-weight: 600; }
    .section-subtitle { font-size: .85rem; color: var(--ink-muted); }
    ...
    /* SALE BADGE */
    .badge-sale { position: absolute; top: .6rem; left: .6rem; background: var(--sale); color: #fff; font-size: .65rem; font-weight: 700; padding: .2rem .55rem; border-radius: 4px; text-transform: uppercase; letter-spacing: .06em; }
    .badge-pct { position: absolute; top: .6rem; right: .6rem; background: var(--accent); color: #fff; font-size: .7rem; font-weight: 700; padding: .2rem .5rem; border-radius: 4px; }
    ...
    .book-price { display: flex; align-items: baseline; gap: .45rem; margin-top: auto; padding-top: .5rem; }
    .price-sale { font-size: 1.1rem; font-weight: 700; color: var(--sale); }
    .price-original { font-size: .82rem; color: var(--ink-muted); text-decoration: line-through; }
    .price-savings { font-size: .72rem; font-weight: 600; color: var(--sale); background: var(--sale-light); padding: .1rem .4rem; border-radius: 4px; margin-left: auto; }

    .book-card-actions { padding: .65rem .85rem; border-top: 1px solid var(--border); display: flex; gap: .5rem; }
    .btn-cart { flex: 1; background: var(--ink); color: #fff; border: none; border-radius: 7px; padding: .45rem .5rem; font-size: .8rem; font-weight: 500; transition: background .15s, transform .15s; }
    .btn-cart:hover { background: var(--accent); transform: translateY(-1px); }
    .btn-cart:active { transform: scale(0.96); }
    .btn-wish { background: none; border: 1.5px solid var(--border); border-radius: 7px; padding: .4rem .55rem; color: var(--ink-muted); font-size: .9rem; transition: border-color .12s, color .12s, transform .2s; }
    .btn-wish:hover { border-color: var(--accent); color: var(--accent); transform: scale(1.15); }

    /* EMPTY STATE */
    .empty-state { text-align: center; padding: 5rem 2rem; color: var(--ink-muted); }
    .empty-state svg { margin: 0 auto 1rem; opacity: .3; }
    .empty-state h3 { font-family: var(--font-display); font-size: 1.3rem; color: var(--ink); margin-bottom: .5rem; }
    ...
    @media(max-width:560px){ nav{padding:0 1rem} .nav-links{display:none} .hero{padding:3rem 1rem 2.5rem} .main-wrap{padding:1rem} .footer-grid{grid-template-columns:1fr} .stats-bar{gap:1.25rem} }
  </style>
</head>
<body>
...
<section class="hero">
  ...
  <div class="hero-content">
    <div class="hero-eyebrow">🔖 Limited Time Deals</div>
    <h1>Big Savings on <em>Great Books</em></h1>
    <p>Stock up on your reading list — hand-picked titles at discounted prices, for a limited time only.</p>
  </div>
</section>

<?php
  $result = mysqli_query($conn, "SELECT * FROM books WHERE original_price IS NOT NULL AND original_price > price ORDER BY (original_price - price) DESC");
  $saleBooks = [];
  ...
  $totalBooks    = count($saleBooks);
  $avgDiscount   = $totalBooks > 0
    ? round(array_sum(array_map(fn($b) => (($b['originalPrice'] - $b['price']) / $b['originalPrice']) * 100, $saleBooks)) / $totalBooks)
    : 0;
  $maxSavings    = $totalBooks > 0
    ? max(array_map(fn($b) => $b['originalPrice'] - $b['price'], $saleBooks))
    : 0;
?>

<div class="stats-bar">
  <span>🏷️ <strong><?= $totalBooks ?></strong> books on sale</span>
  <span>💰 Save up to <strong>PHP <?= number_format($maxSavings, 2) ?></strong></span>
  <span>📉 Average <strong><?= $avgDiscount ?>%</strong> off</span>
</div>
...
<div class="toolbar">
  <div class="search-wrap">
    ...
    <input type="text" id="search-input" placeholder="Search sale books…" oninput="filterBooks()">
  </div>
  <select class="filter-select" id="sort-select" onchange="filterBooks()">
    <option value="savings-desc">Biggest Savings First</option>
    <option value="pct-desc">Highest % Off First</option>
...
<div class="main-wrap">
  <div class="section-header">
    ...
    <span class="section-subtitle">Prices reduced from original</span>
  </div>
...
function savings(b) {
  return b.originalPrice - b.price;
}
...
function filterBooks() {
  ...
  if (sort === 'savings-desc') filtered.sort((a, b) => savings(b) - savings(a));
  if (sort === 'pct-desc')     filtered.sort((a, b) => pct(b) - pct(a));
  ...
  if (!books.length) {
    grid.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1">
        <svg width="48" height="48" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M20 12V22H4V12"/><path d="M22 7H2v5h20V7z"/><path d="M12 22V7"/><path d="M12 7H7.5a2.5 2.5 0 010-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 000-5C13 2 12 7 12 7z"/></svg>
        <h3>No sale books found</h3>
        <p>Try a different search term or check back soon for new deals.</p>
      </div>`;
    return;
  }

  grid.innerHTML = books.map((b, i) => {
    const discount = pct(b);
    const saved    = savings(b);
    return `
    <div class="book-card" style="animation: scaleIn 0.4s ease ${i * 0.06}s both">
      <div class="book-cover">
        ...
        <span class="badge-pct">−${discount}%</span>
      </div>
      <div class="book-info">
        ...
        <div class="book-price">
          <span class="price-sale">${fmtPrice(b.price)}</span>
          <span class="price-original">${fmtPrice(b.originalPrice)}</span>
          <span class="price-savings">Save ${fmtPrice(saved)}</span>
        </div>
      </div>
...
function updateCartUI() {
  ...
  container.innerHTML = cart.map(({book, qty}) => `
    <div class="cart-item">
      ...
      <button class="cart-item-remove" onclick="removeFromCart(${book.id})">✕</button>
    </div>`).join('');
}
